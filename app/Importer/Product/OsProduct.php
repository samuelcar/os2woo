...
class OsProduct extends Model implements ToWooCommerce
{
    ...
    protected $image_path = 'http://www.motorsportsuperstore.com.au//catalog/images/';
    ...
    public function categories() {
        return $this->belongsToMany(OsCategory::class, 'products_to_categories', 'products_id', 'categories_id');
    }
    ...
    public function toWooCommerce() {
        return ["title" => $this->description->products_name, "type" => count($this->getOsAttributes()) > 1 ? 'variable' : 'simple', "status" => $this->products_status ? 'publish' : 'draft',
        
        //decided draft because can be seen in backend and not in frontend
        "downloadable" => false, "virtual" => false, "sku" => $this->products_model, "regular_price" => $this->products_price, "sale_price" => isset($this->specials->specials_new_products_price) ? $this->specials->specials_new_products_price : '', "sale_price_dates_from" => isset($this->specials->start_date) ? $this->specials->start_date : '', "sale_price_dates_to" => isset($this->specials->start_date) ? $this->specials->start_date : '', "tax_status" => 'taxable', "tax_class" => '',
        
        //            "managing_stock" => $wc_product->managing_stock(), //maybe optional
        "stock_quantity" => intval($this->description->products_quantity), "in_stock" => (bool)$this->products_inStock,
        ...
        "description" => isset($this->description->products_description) ? $this->description->products_description : '', "enable_html_description" => true,
        ...
        'images' => [['src' => $this->image_path . $this->products_image, 'position' => 0]],
        ...
        "attributes" => $this->getWooAttributes(), "variations" => $this->getVariations()
    ...
    public function getVariations() {
        ...
        if (count($attributes) == 1) {
            ...
            foreach ($options as $opt) {
                ...
                $actual["attributes"] = [["name" => $current['name'], "slug" => $current['slug'], "option" => trim($opt) ]];
                $variations[] = $actual;
            }
        }
    ...
    public function getDefaultWooAttribute() {
        ...
        if (count($attributes) == 1) {
            ...
            $options = isset($current['options']) ? $current['options'] : [];
            foreach ($options as $opt) {
                $actual['default_attributes'] = [['name' => $current['name'], 'slug' => $current['slug'], 'option' => $opt]];
                break;
            }
        }
    ...
    private function getProductCategories() {
        ...
        while ($continue) {
            ...
            $categories[] = $firstCat->description->woo_id;
